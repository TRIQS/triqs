...
class test_brillouin_zone(unittest.TestCase):
    ...
    def test_mesh_construction(self):
        ...
        def run(units, per_mat):
            ...
            p1, p2, p3 = per_mat
            k1, k2, k3 = bz.units
            assert np.array_equal(k_mesh.closest_index(k1), p1)
            assert np.array_equal(k_mesh.closest_index(k2), p2)
            assert np.array_equal(k_mesh.closest_index(k3), p3)
            assert linalg.norm(dot(bl.units, bz.units.T) - eye(3)*2*pi) < 1e-14

        # ---- square lattice : 16 x 16 x 1
        units = np.array([[1.,0.,0.],[0.,1.,0.],[0.,0.,1.]])
        per_mat = np.array([[16, 0, 0],[0, 16, 0],[0, 0, 1]])
        run(units, per_mat)

        # ---- triangular lattice : 6 x 6 x 1
        units = np.array([[1.,0.,0.],[0.5,np.sqrt(3)/2.,0.],[0.,0.,1.]])
        per_mat = np.array([[6, 0, 0],[0, 6, 0],[0, 0, 1]])
        run(units, per_mat)
        ...
        per_mat = np.diag(np.random.randint(1, 10, size=(3,)))
    ...
    def test_interpolate(self):
        ...
        n_k = 40
        per_mat = np.array([[n_k, 0, 0],[0, n_k, 0],[0, 0, 1]])
        k_mesh  = MeshBrZone(bz, per_mat)
        ...
        n_k_fine = 5 * n_k
        per_mat = np.array([[n_k_fine, 0, 0],[0, n_k_fine, 0], [0, 0, 1]])
