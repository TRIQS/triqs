...
int main() {
 ...
 std::cout << "Gw2(w0)=" << Gw2(w0) << std::endl<< std::endl;
 ...
 std::cout << "Giw(w0)(0,0)=" << Giw(w0)(0,0) << std::endl;
 if ( std::abs(Gw(w0)(0,0)-w0) > precision) TRIQS_RUNTIME_ERROR<< "evaluator error in Giw\n";  
 std::cout << "Giw2(w0)=" << Giw2(w0) << std::endl<< std::endl;
 if ( std::abs(Gw2(w0)-w0) > precision) TRIQS_RUNTIME_ERROR<< "evaluator error in Giw2\n";  
 ...
 std::cout << "Git2(it0-beta)=" << Git2(it0-beta) << std::endl<< std::endl; 
 if ( std::abs(Git2(it0-beta)+it0) > precision) TRIQS_RUNTIME_ERROR<< "evaluator error in Git3\n";   
 
 //verification of windowing for imaginary times near the boundaries of [0,beta] for half_bins
 double delta = Git2.mesh().delta();
 std::cout << "Git2("<<delta/4<<")=" << Git2(delta/4) << std::endl; 
 if ( std::abs(Git2(delta/4)-Git2(delta/2)) > precision) TRIQS_RUNTIME_ERROR<< "evaluator error in Git3\n";   
 std::cout << "Git2("<<beta-delta/4<<")=" << Git2(beta-delta/4) << std::endl; 
 if ( std::abs(Git2(beta-delta/4)-Git2(beta-delta/2)) > precision) TRIQS_RUNTIME_ERROR<< "evaluator error in Git3\n";  
