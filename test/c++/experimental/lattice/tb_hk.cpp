...
TEST(tb_tests, simple_construct) { // NOLINT
  ...
  auto tb              = tb_hk(displ_vec, overlap_mat_vec);
...
TEST(tb_tests, test_evaluators) { // NOLINT
  ...
  double t             = 1.0;
  auto overlap_mat_vec = std::vector(displ_vec.size(), nda::array<dcomplex, 2>(nda::diag({t, t})));
  auto tb_H            = tb_hk(displ_vec, overlap_mat_vec);
...
TEST(tb_tests, tb_adaptive_test) { // NOLINT
  ...
  // adaptive
  auto int_1d_adapt = triqs::experimental::utility::integrate_1d_adapt<nda::matrix<dcomplex>>{1e-5};
...
TEST(tb_tests, h5_read_write) {
  ...
  // read
  {
    ...
    auto grp   = h5::group{file};
    auto tb_in = h5::h5_read<tb_hk>(grp, "hamiltonian");
    EXPECT_EQ(tb, tb_in);
  }
}
...
TEST(tb_tests, mpi_broadcast) {
  ...
  double t             = 1.0;
  auto overlap_mat_vec = std::vector(displ_vec.size(), nda::array<dcomplex, 2>(nda::diag({t, t})));
  auto tb_ref          = tb_hk(displ_vec, overlap_mat_vec);

  // Only rank 0 has the actual data, others have default-constructed object
  auto tb = world.rank() == 0 ? tb_ref : tb_hk{};
