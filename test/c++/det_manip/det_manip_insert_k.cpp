...
// Test the insert_k operation of det_manip.
template <typename T> void test_insert_k(int k) {
...
TEST(TRIQSDetManip, InsertKDouble) {
  for (int i = 1; i < 5; ++i) test_insert_k<double>(i);
}

TEST(TRIQSDetManip, InsertKComplex) {
  for (int i = 1; i < 5; ++i) test_insert_k<std::complex<double>>(i);
}
