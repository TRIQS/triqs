...
struct test {
  ...
  void check() {
#ifndef PRINT_ALL
    std::cerr << "det = " << D.determinant() << " == " << std::complex<double>(nda::linalg::det(D.matrix())) << std::endl;
#else
    std::cerr << "det = " << D.determinant() << " == " << std::complex<double>(nda::linalg::det(D.matrix())) << std::endl
              << D.inverse_matrix() << D.matrix() << nda::matrix<std::complex<double>>(nda::linalg::inv(D.matrix())) << std::endl;
