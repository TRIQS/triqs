...
namespace triqs {
  ...
  */
      template <typename ValueType, bool Weak = false> class shared_block {
        ...
        void construct_delegate(size_t size) {
          ...
          if (size == 0) {
            ...
            sptr  = new mem_block<ValueType>(size);
            data_ = sptr->p;
          }
        }
        ...
        explicit shared_block(size_t size, tags::_allocate_only) {
          ...
          if (size == 0) {
            ...
            sptr  = new mem_block<ValueType>(size, tags::_allocate_only{});
            data_ = sptr->p;
          }
        }

        void _init_raw(size_t i, ValueType &&x) {
          assert(sptr);
          sptr->_init_raw(i, std::move(x));
        }
        ...
        explicit shared_block(PyObject *arr, bool weak) : sptr(new mem_block<ValueType>(arr, weak)) {
          data_ = sptr->p;
          s     = sptr->size();
        }
#endif

        explicit shared_block() {
          sptr  = nullptr;
          data_ = nullptr;
          s     = 0;
        }

        shared_block(shared_block const &X) noexcept {
          sptr  = X.sptr;
          data_ = X.data_;
          s     = X.s;
          if (sptr) inc_ref<Weak>(sptr);
        }
        shared_block(shared_block<ValueType, !Weak> const &X) noexcept {
          sptr  = X.sptr;
          data_ = X.data_;
          s     = X.s;
          if (sptr) inc_ref<Weak>(sptr);
        ...
        shared_block(shared_block &&X) noexcept {
          sptr   = X.sptr;
          data_  = X.data_;
          s      = X.s;
          X.sptr = nullptr;
        ...
        shared_block clone() const {
          ...
          shared_block res;
          if (!empty()) {
            res.sptr  = new mem_block<ValueType>(*sptr);
            res.data_ = res.sptr->p;
            res.s     = s;
          }
          return res;
        }
        ...
        template <class Archive> void save(Archive &ar, const unsigned int version) const { ar << TRIQS_MAKE_NVP("ptr", sptr); }
        template <class Archive> void load(Archive &ar, const unsigned int version) {
          if (sptr) dec_ref<Weak>(sptr);
          ar >> TRIQS_MAKE_NVP("ptr", sptr);
          data_ = (sptr ? sptr->p : nullptr);
          s     = (sptr ? sptr->size() : 0);
          //if (sptr) inc_ref<Weak>(sptr);
        }
