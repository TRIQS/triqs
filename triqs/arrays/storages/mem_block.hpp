...
namespace triqs {
  namespace arrays {
    namespace storages { //namespace details {
      ...
      template <bool Weak, typename ValueType> DISABLE_IFC(Weak) dec_ref(mem_block<ValueType> *const m) {
        ...
        if (m->ref_count == 0) {
          ...
          if (m->weak_ref_count != 0)
            TRIQS_RUNTIME_ERROR << "Deleting an memory block of an array with still " << m->weak_ref_count << " weak references";
#endif
          delete m;
        }
      }
...
#define TRIQS_PRINT_MEMORY_USED std::cerr << "[triqs:mem_block]" << triqs::arrays::storages::__global_memory_allocated_from_c.count << std::endl;
#else
#define TRIQS_MEMORY_USED_INC(s)
	...
	std::atomic<size_t> weak_ref_count; // number of refs. :  >=1
        ...
        mem_block(size_t s) : size_(s), py_numpy(nullptr), py_guard(nullptr) {
          try {
            ...
            TRIQS_RUNTIME_ERROR << "Memory allocation error in memblock construction. Size :" << s << "  bad_alloc error : " << ba.what();
          }
        ...
        mem_block(PyObject *obj, bool python_object_is_borrowed) {
          TRACE_MEM_DEBUG(" construct memblock from pyobject" << obj << " # ref =" << obj->ob_refcnt
                                                              << " python_object_is_borrowed = " << python_object_is_borrowed);
          assert(obj);
          import_numpy_array();
        ...
        ~mem_block() { // delete memory manually iif py_obj is not set. Otherwise the python interpreter will do that for us.
          TRACE_MEM_DEBUG("deleting mem block p =" << p << "  py_obj = " << py_numpy << "    ref of py obj if exists"
                                                   << (py_numpy ? py_numpy->ob_refcnt : -1));
          assert(ref_count <= 1);
          assert(py_guard == nullptr); // state 3 forbidden
          if (py_numpy)
        ...
        mem_block(mem_block &&X) noexcept {
          ...
          weak_ref_count = X.weak_ref_count;
          py_numpy       = X.py_numpy;
          py_guard       = X.py_guard;
          X.p            = nullptr;
          X.py_numpy     = nullptr;
          X.py_guard     = nullptr; // state 0, ready to destruct
        }
        ...
        mem_block(mem_block const &X) : size_(X.size()), py_numpy(nullptr), py_guard(nullptr) {
          try {
            p = new ValueType[X.size()];
          } catch (std::bad_alloc &ba) {
            TRIQS_RUNTIME_ERROR << "Memory allocation error in memblock copy construction. Size :" << X.size() << "  bad_alloc error : " << ba.what();
          }
        ...
        static void delete_python_guard(void *ptr) {
          ...
          assert(m->py_guard != nullptr);
          assert(m->py_numpy == nullptr);
          m->py_guard = nullptr;
        ...
        template <class Archive> void load(Archive &ar, const unsigned int version) {
          ...
          try {
            ...
            TRIQS_RUNTIME_ERROR << "Memory allocation error in memblock deserialization. Size :" << size_ << "  bad_alloc error : " << ba.what();
          }
