 ...
 */
 template <int Rank, typename TraversalOrder=void> class map {
   ...
   map(domain_type const & C, memory_layout_t<Rank> ml): mydomain(C), start_shift_(0), memory_order_(std::move(ml)) {compute_stride_compact();}
...
bool raw_copy_possible(const cuboid::map<R1, To1>& X1, const cuboid::map<R2, To2>& X2) {
  return ( (X1.memory_layout() == X2.memory_layout())
