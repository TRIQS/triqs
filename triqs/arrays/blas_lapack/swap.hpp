...
namespace triqs {
  namespace arrays {
    namespace blas {
      ...
      namespace f77 { // overload
        ...
        inline void swap(const int &N, double *x, const int &incx, double *Y, const int &incy) { TRIQS_FORTRAN_MANGLING(dswap)(N, x, incx, Y, incy); }
  ...
  */
      ...
      typename std::enable_if<is_blas_lapack_type<typename VTX::value_type>::value && have_same_value_type<VTX, VTY>::value>::type swap(VTX &X,
                                                                                                                                        VTY &Y) {
