...
namespace triqs {
  namespace h5 {
    ...
    namespace {
      ...
      template <typename T> inline void h5_write_vector_impl(group g, std::string const &name, std::vector<T> const &V) {
        ...
        if (H5Sget_simple_extent_npoints(d_space) > 0) {
          auto err = H5Dwrite(dset, h5::data_type_memory<T>(), d_space, H5S_ALL, H5P_DEFAULT, &V[0]);
    ...
    void h5_write(group f, std::string const &name, std::vector<long> const &V) { h5_write_vector_impl(f, name, V); }
    ...
    void h5_read(group f, std::string const &name, std::vector<long> &V) { h5_read_impl(f, name, V); }
