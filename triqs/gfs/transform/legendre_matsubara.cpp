...
namespace triqs {
  namespace gfs {
    ...
    void legendre_matsubara_direct(gf_view<imfreq> gw, gf_const_view<legendre> gl) {
      ...
      for (auto om : gw.mesh()) {
        for (auto l : gl.mesh()) { gw[om] += legendre_T(om.index(), l.index()) * gl[l]; }
      }
    }
    ...
    void triqs_gf_view_assign_delegation(gf_view<imfreq> gw, gf_keeper<tags::legendre, legendre> const &L) { legendre_matsubara_direct(gw, L.g); }
    void triqs_gf_view_assign_delegation(gf_view<imtime> gt, gf_keeper<tags::legendre, legendre> const &L) { legendre_matsubara_direct(gt, L.g); }
    void triqs_gf_view_assign_delegation(gf_view<legendre> gl, gf_keeper<tags::legendre, imfreq> const &L) { legendre_matsubara_inverse(gl, L.g); }
    void triqs_gf_view_assign_delegation(gf_view<legendre> gl, gf_keeper<tags::legendre, imtime> const &L) { legendre_matsubara_inverse(gl, L.g); }
  } // namespace gfs
} // namespace triqs
