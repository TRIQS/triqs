...
namespace gfs {
 ...
 class gf_const_view : TRIQS_CONCEPT_TAG_NAME(ImmutableGreenFunction) {
  ...
  using view_type = gf_const_view<Mesh, Target, Singularity, Evaluator>;
  ...
  private:
  template <typename G>
  gf_const_view(G &&x, bool) // bool to disambiguate
      : _mesh(x.mesh()),
  ...
  gf_const_view(impl_tag, M &&m, D &&dat, S &&sing, SY &&sy, indices_t ind, std::string name)
  ...
  /// Move
  gf_const_view(gf_const_view &&) = default; // TO BE REWRITTEN ??
  ...
  clef::make_expr_call_t<gf_const_view const &, Args...> operator()(Args &&... args) const &{
   return clef::make_expr_call(*this, std::forward<Args>(args)...);
  }
  ...
  clef::make_expr_subscript_t<gf_const_view const &, Arg> operator[](Arg &&arg) const &{
  ...
  public:
  _on_mesh_wrapper_const friend on_mesh(gf_const_view const &f) {
   return {f};
  }
  _on_mesh_wrapper friend on_mesh(gf_const_view &f) {
   return {f};
  }

  //----------------------------- HDF5 -----------------------------

  friend std::string get_triqs_hdf5_data_scheme(gf_const_view const &g) {
  ...
  /// Write into HDF5
  friend void h5_write(h5::group fg, std::string subgroup_name, gf_const_view const &g) {
  ...
  /// Read from HDF5
  friend void h5_read(h5::group fg, std::string subgroup_name, gf_const_view &g) {
