...
struct tag_is_tail {};
template <typename T>
constexpr bool is_tail_v = std::is_base_of<tag_is_tail, std::decay_t<T>>::value;

/**
 * The high-frequency behavior (or **tail**) of a Green's function is of
 *paramount importance in the context of Fourier
 *transformations: since fermionic Green's functions are slowly decaying
 *functions of frequency, special care has to be taken
 *when Fourier-transforming them to time.
 *
 * The "tail" of a Green's function encodes the behavior of the Green's function
 *$G$ at large Matsubara frequencies, namely
 *
 * $$\mathbf{G}(i\omega_n) \stackrel {=}{\infty} \mathbf{a}_{-1}\cdot i\omega_n
 *+ \mathbf{a}_{0} +\mathbf{a}_{1}\cdot \frac{1}{
...
public:
  ...
  using crv_t =
      typename const_view_type_if_exists_else_type<typename T::slice_t>::type;
  // using crv_t = std14::conditional_t<is_scalar_target, dcomplex,
  // const_view_or_type_t<typename T::slice_t>>;
  using rv_t = std14::conditional_t<is_scalar_target, dcomplex &,
                                    view_or_type_t<typename T::slice_t>>;
...
public:
  /**
   3-dim array of the coefficients: data(i,n,m) stands for
   $(\mathbf{a}_{i+o_\mathrm{min}})_{nm}$
  */
  ...
  */
  ...
  template <typename ArrayType,
            typename Enable = std14::enable_if_t<
                arrays::ImmutableCuboidArray<ArrayType>::value>>
  ...
  explicit __tail(mini_vector<Int, T::rank> sh)
      : _data(sh.front_append(_size())) {
    zero();
  }
  ...
  /**
    ...
    */
  template <typename Int>
  explicit __tail(mini_vector<Int, T::rank> sh, int n_orders, int o_min)
      : __tail{sh} {
    if (o_min < order_min())
      TRIQS_RUNTIME_ERROR << "Tail construction : o_min is < " << order_min();
  ...
  explicit __tail(int n0, Args const &... args) {
    ...
    static_assert((n_args - T::rank == 2) or ((n_args - T::rank == 0)),
                  "Too many arguments in tail construction");
    ...
    *this = (n_args == T::rank
                 ? __tail{shape}
                 : __tail{shape, _args[n_args - 2], _args[n_args - 1]});
  }
  ...
  /// FIXME : only a shape ?
  /// Returns a omega. Args is anything from which to construct a tail (shape,
  /// tail to copy ...).
  template <typename... Args> static __tail omega(Args const &... args) {
    __tail t(args...);
    t.data()() =
        0; // we can pass a tail for a copy, so we can not be sure data is 0.
  ...
  __tail &operator=(std::complex<double> const &x) {
    _data() = 0.0;
    rv_t(_data(0, ellipsis())) =
        x; // to have the matrix unit correct if matrix valued
    return *this;
  }
  ...
  __tail &operator=(regular_type const &x) {
    return operator=(const_view_type{x});
  }
  __tail &operator=(view_type const &x) {
    return operator=(const_view_type{x});
  }
  ...
  bool _isnan(int order) const {
    return any(isnan(_data(order - order_min(), ellipsis{})));
  }

  // Is the element at this order > 0. DOES NOT CHECK NaN.
  bool is_non_zero(int order) const {
    return (max_element(abs(_data(order - order_min(), ellipsis{}))) > 1.e-10);
  }
  ...
  /// Returns the last order which is non nan and  order_min() - 1 if tail is
  /// nan.
  ...
  int n_valid_orders() const {
    int om = order_min();
    while ((om <= order_max()) and (!_isnan(om))) {
      ++om;
    }
    return om - order_min();
  }
  ///
  arrays::mini_vector<int, T::rank> target_shape() const {
    return _data.shape().front_pop();
  }

  /// true iif the tail decreasing at infinity
  bool is_decreasing_at_infinity() const {
    for (int om = order_min(); om <= 0; ++om) {
      if (_isnan(om) or (is_non_zero(om)))
        return false;
    }
    return true;
  }
...
public:
  ...
  rv_t operator()(int n) {
    if (n > order_max())
      TRIQS_RUNTIME_ERROR << " n > Max Order. n= " << n
                          << ", Max Order = " << order_max();
    if (n < order_min())
      TRIQS_RUNTIME_ERROR << " n < Min Order. n= " << n
                          << ", Min Order = " << order_min();
...
public:
  ...
  friend void h5_read(h5::group fg, std::string subgroup_name, __tail &t) {
    ...
    if (!gr.has_key(
            "mask")) { // if no mask, we have the latest version of the tail
      h5_read(gr, "data", t._data); // Add here backward compat code IF
                                    // order_min/max where to change
    } else {
      // backward compatibility code
      int omin = h5::h5_read<int>(gr, "omin");
      auto d = h5::h5_read<typename data_t::regular_type>(gr, "data");
      if (omin < t.order_min())
        TRIQS_RUNTIME_ERROR << "Reading h5 file for a tail : order_min is < -2 "
      ...
      // std::cout << omin - t.order_min() <<" ---- "<< omax + 1 -
      // t.order_min()<< " ---"<<  omax - omin + 2 << std::endl;
      auto _ = ellipsis();
      t.reset(omax + 1);
      t._data(range(omin - t.order_min(), omax + 1 - t.order_min()), _) =
          d(range(0, omax - omin + 1), _);
    }
  }
  ...
  /// mpi broadcast
  friend void mpi_broadcast(__tail &t, mpi::communicator c = {}, int root = 0) {
    mpi_broadcast(t._data);
  }

  ///
  friend std::ostream &operator<<(std::ostream &out, __tail const &x) {
    return out << const_view_type{x};
  }

  /// Conj
  friend regular_type conj(__tail const &x, bool imaginary) {
    return conj(const_view_type{x}, imaginary);
  }

  ///
  friend regular_type transpose(__tail const &x) {
    return transpose(const_view_type{x});
  }
};
...
public:
  ...
  */
  ...
  /// Swap
  friend void swap(__tail_view &a, __tail_view &b) noexcept {
  ...
  __tail_view &operator=(regular_type const &x) {
    return operator=(const_view_type{x});
  }
  __tail_view &operator=(view_type const &x) {
    return operator=(const_view_type{x});
  }
...
public:
  ...
  /// mpi broadcast
  friend void mpi_broadcast(__tail_view &t, mpi::communicator c = {},
                            int root = 0) {
    mpi_broadcast(t._data);
  }

  ///
  friend std::ostream &operator<<(std::ostream &out, __tail_view const &x) {
    return out << const_view_type{x};
  }

  /// Conj
  friend regular_type conj(__tail_view const &x, bool imaginary) {
    return conj(const_view_type{x}, imaginary);
  }

  ///
  friend regular_type transpose(__tail_view const &x) {
    return transpose(const_view_type{x});
  }
};
...
public:
  ...
  */
  ...
  /// Swap
  friend void swap(__tail_const_view &a, __tail_const_view &b) noexcept {
    swap(a._data, b._data);
  }
...
public:
  ...
  /// read from h5
  friend void h5_read(h5::group fg, std::string subgroup_name,
                      __tail_const_view &t) {
    auto gr = fg.open_group(subgroup_name);
    if (!gr.has_key(
            "mask")) { // if no mask, we have the latest version of the tail
  ...
  /// mpi broadcast
  friend void mpi_broadcast(__tail_const_view &t, mpi::communicator c = {},
                            int root = 0) {
    mpi_broadcast(t._data);
  }
  ...
  friend std::ostream &operator<<(std::ostream &out, __tail_const_view x) {
    if (x.empty())
      return out << "empty tail" << std::endl;
    for (int u = x.order_min(); u <= x.order_max() and (!x._isnan(u)); ++u)
      out << "\n ...  Order " << u << " = " << x(u);
    return out;
  }
  ...
  friend regular_type conj(__tail_const_view t, bool imaginary) {
    ...
    for (int j = 0; j < t._size(); j++)
      r.data()(j, ellipsis()) =
          (((j - order_min()) % 2 == 1 && imaginary) ? -1 : 1) *
          conj(r.data()(j, ellipsis()));
    return r;
  }

  /// transpose
  friend regular_type transpose(__tail_const_view t) {
    return {transposed_view(t.data(), 0, 2, 1)};
  }
};
...
template <typename T>
auto mpi_reduce(__tail_const_view<T> t, mpi::communicator c = {}, int root = 0,
...
template <typename T>
auto mpi_reduce(__tail<T> const &t, mpi::communicator c = {}, int root = 0,
                bool all = false, MPI_Op op = MPI_SUM) {
  return mpi_reduce(__tail_const_view<T>{t}, c, root, all, op);
}

template <typename T>
auto mpi_reduce(__tail_view<T> t, mpi::communicator c = {}, int root = 0,
                bool all = false, MPI_Op op = MPI_SUM) {
  return mpi_reduce(__tail_const_view<T>{t}, c, root, all, op);
}
...
template <typename T>
auto evaluate(__tail_const_view<T> t, dcomplex const &omega) {
...
#else
  static_if(
      bool_<T::rank != 0>{}) // if not a number, need to resize and put to 0
      .then([&](auto &rr) {
  ...
  for (int u = t.largest_non_nan(); u >= t.order_min(); --u)
    r = r / omega + t(u); // NB () returns a matrix if matrix_valued
...
/// Evaluate the tail to  $\sum_{n={ordermin}}^{ordermax} M_n/{\omega}^n$
template <typename T>
auto evaluate(__tail_view<T> const &t, dcomplex const &omega) {
  return evaluate(__tail_const_view<T>{t}, omega);
}
...
/// Composition
template <typename T>
__tail<T> __compose(__tail_const_view<T> x, __tail_const_view<T> t) {
...
std14::enable_if_t<is_tail_v<A> and is_tail_v<B>, typename A::regular_type>
...
template <typename T> __tail<T> inverse(__tail_const_view<T> const &t) {
  ...
  __tail<T> res = t;
  if (t.empty() or
      (omin1 == -t.order_max() - 1)) { // is not invertible. return nan
    res.reset();
    return res;
  }
  res.data()() = 0;
...
/// Inverse of the tail
template <typename T> __tail<T> inverse(__tail<T> const &x) {
  return inverse(__tail_const_view<T>{x});
}

/// Inverse of the tail
template <typename T> __tail<T> inverse(__tail_view<T> const &x) {
  return inverse(__tail_const_view<T>{x});
}
...
// If the placeholder of the expression is a placeholder of rank > 0, we do
// nothing...
template <typename T, typename Expr, int... Is>
void assign_singularity_from_function(
    __tail_view<T> t, clef::make_fun_impl<Expr, Is...> const &rhs) {
  static_assert(sizeof...(Is) == 1, "???"); // one var only
  // We assign iif the placeholder of the expression has rank 0
  constexpr bool ph_has_rank0 =
      clef::get_placeholder_rank(clef::_ph<Is...>()) == 0;
#ifdef __cpp_if_constexpr
  if
...
void assign_singularity_from_function(__tail<T> &t, RHS const &rhs) {
...
__tail_const_view<T> slice_target_sing(__tail<T> const &t,
                                       Args const &... args) {
...
///
template <typename T, typename... Args>
__tail_const_view<T> slice_target_sing(__tail_const_view<T> t,
                                       Args const &... args) {
  return {t.data()(arrays::range{}, args...)};
}
...
///
template <typename T, typename... Args>
__tail_view<T> slice_target_sing(__tail<T> &t, Args const &... args) {
  return {t.data()(arrays::range{}, args...)};
}

///
template <typename T, typename... Args>
__tail_view<T> slice_target_sing(__tail_view<T> t, Args const &... args) {
  return {t.data()(arrays::range{}, args...)};
}
...
template <typename T, typename... Args>
__tail_view<scalar_valued> slice_target_to_scalar_sing(__tail_view<T> t,
                                                       Args const &... args) {
  return {t.data()(arrays::range{}, args...)};
}
...
///
template <typename T, typename... Args>
__tail_const_view<scalar_valued>
slice_target_to_scalar_sing(__tail_const_view<T> t, Args const &... args) {
  return {t.data()(arrays::range{}, args...)};
}

template <typename T, typename... Args>
__tail_view<scalar_valued> slice_target_to_scalar_sing(__tail<T> &t,
                                                       Args const &... args) {
  return {t.data()(arrays::range{}, args...)};
}
...
template <typename T, typename... Args>
__tail_const_view<scalar_valued>
slice_target_to_scalar_sing(__tail<T> const &t, Args const &... args) {
  return {t.data()(arrays::range{}, args...)};
}
...
template <typename T>
constexpr bool is_scalar_or_array_v =
    arrays::ImmutableArray_v<T> || utility::is_in_ZRC<T>::value;

template <typename T> constexpr bool is_matrix_v = arrays::ImmutableMatrix_v<T>;

template <typename T>
constexpr bool is_scalar_or_array_or_matrix_v =
    arrays::ImmutableCuboidArray_v<T> || utility::is_in_ZRC<T>::value;
...
template <typename A, typename B>
std14::enable_if_t<is_tail_v<A> and is_scalar_or_array_or_matrix_v<B>,
                   typename A::regular_type>
operator+(A const &a, B const &b) {
...
template <typename A, typename B>
std14::enable_if_t<is_scalar_or_array_or_matrix_v<A> and is_tail_v<B>,
                   typename B::regular_type>
operator+(A const &a, B const &b) {
...
template <typename T>
__tail<T> operator*(__tail_const_view<T> const &l,
                    __tail_const_view<T> const &r) {
  ...
  for (int n = res.order_min(); n <= res.order_max(); ++n) {
    // sum_{p}^n a_p b_{n-p}. p <= a.n_max, p >= a.n_min and n-p <=b.n_max and
    // n-p >= b.n_min
...
template <typename A, typename B>
std14::enable_if_t<is_matrix_v<A> and is_tail_v<B>, typename B::regular_type>
operator*(A const &a, B const &b) {
  typename B::regular_type res(
      make_shape(first_dim(a), b.data().shape()[1 + 1]));
  for (int n = b.order_min(); n <= b.order_max(); ++n)
    res(n) = a * b(n);
  return res;
}

template <typename A, typename B>
std14::enable_if_t<is_tail_v<A> and is_matrix_v<B>, typename A::regular_type>
operator*(A const &a, B const &b) {
  typename A::regular_type res(
      make_shape(a.data().shape()[1 + 0], second_dim(b)));
  for (int n = a.order_min(); n <= a.order_max(); ++n)
    res(n) = a(n) * b;
  return res;
}
...
template <typename A, typename B>
std14::enable_if_t<is_scalar_or_array_v<A> and is_tail_v<B>,
                   typename B::regular_type>
operator*(A const &a, B const &b) {
  typename B::regular_type res = b;
  for (int n = b.order_min(); n <= b.order_max(); ++n)
...
template <typename A, typename B>
std14::enable_if_t<is_tail_v<A> and is_scalar_or_array_v<B>,
                   typename A::regular_type>
operator*(A const &a, B const &b) {
  typename A::regular_type res = a;
  for (int n = a.order_min(); n <= a.order_max(); ++n)
...
template <typename A, typename B>
std14::enable_if_t<is_tail_v<B> and is_tail_v<A>, typename B::regular_type>
operator/(A const &a, B const &b) {
  return a * inverse(b);
}

template <typename A, typename B>
std14::enable_if_t<is_tail_v<B> and is_in_ZRC<A>::value,
                   typename B::regular_type>
operator/(A const &a, B const &b) {
  return a * inverse(b);
}
...
template <typename T, typename U> void operator+=(__tail_view<T> g, U &&x) {
  g = g + x;
}
template <typename T, typename U> void operator-=(__tail_view<T> g, U &&x) {
  g = g - x;
...
template <typename T, typename U> void operator+=(__tail<T> &g, U &&x) {
  g = g + x;
}
template <typename T, typename U> void operator-=(__tail<T> &g, U &&x) {
  g = g - x;
...
inline __tail_view<matrix_valued>
reinterpret_as_matrix_valued_sing(__tail_view<scalar_valued> t) {
  return {reinterpret_array_add_1x1(t.data())};
}
inline __tail_const_view<matrix_valued>
reinterpret_as_matrix_valued_sing(__tail_const_view<scalar_valued> t) {
  return {reinterpret_array_add_1x1(t.data())};
}

// ----- make_tail_view_from_data. make a tail_view from the data, determining
// if it is a const_view or not
//
// if A is a const_view or A is passed by const &, then the proxy is const,
// otherwise it is not.
template <typename A>
std14::conditional_t<std14::decay_t<A>::is_const ||
                         std::is_const<std14::remove_reference_t<A>>::value,
                     __tail_const_view<target_from_array<A, 1>>,
                     __tail_view<target_from_array<A, 1>>>
