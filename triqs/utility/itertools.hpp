...
namespace triqs::utility {
  ...
  namespace details {
    ...
    template <typename... It> struct zip_iter : iterator_facade<zip_iter<It...>> {
      ...
      template <size_t... Is> auto tie_map_impl(std::index_sequence<Is...>) const { return std::tuple<decltype(*std::get<Is>(its))...>(*std::get<Is>(its)...); }
      decltype(auto) dereference() const { return tie_map_impl(std::index_sequence_for<It...>{}); }
    };
   ...
   * This function returns itself a range.
   *
   ...
   */
  // template <typename... T> auto zip(T&&... ranges) { return details::zipped<T...>{std::tie(std::forward<T>(ranges)...)}; }
   ...
   * @param ranges The ranges to zip. Note: They have to be of equal length!
   */
  //template <typename... T> auto prod(T&&... ranges) { return details::multiplied<T...>{std::tie(std::forward<T>(ranges)...)}; }
