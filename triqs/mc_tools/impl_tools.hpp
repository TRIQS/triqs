...
namespace triqs { namespace mc_tools {
 ...
 template<typename T> concept bool H5ReadWritable = requires(T x) { 
  {h5_read(h5::group{},std::string{}, (T&)x} -> void;
  {h5_write(h5::group{},std::string{}, (T const&)x} -> void;
 };
 ...
 template <typename T>
 struct synth_collect_statistics<T, decltype(std::declval<T>().collect_statistics(triqs::mpi::communicator()))> {
