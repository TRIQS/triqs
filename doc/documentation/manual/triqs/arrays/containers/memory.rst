...
View classes contains a documentation/manual/triqs counting system to the memory block they view
...
* Weak views are completely similar to "normal" views, except that they do `not` increase the memory
  documentation/manual/triqs counter.
...
* Normal views are be implicitely constructed from weak view (the reverse is also true), 
  ...
  the documentation/manual/triqs counting is again incremented, and the memory guarantee holds.
...
* The () operator returns a weak view `on a fully compliant C++11 compiler` (in particular, `rvalue documentation/manual/triqs for *this` must be implemented)
