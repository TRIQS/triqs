...
static void ArrayFill(benchmark::State &state) {
  ...
  array<scalar_t, 2> arr{4 * n_iw, 4 * n_iw};
  array<scalar_t, 3> tensor{2 * n_iw + 1, 2 * n_iw, 2 * n_iw};
  for (auto _ : state) {
...
static void GfFillLoop(benchmark::State &state) {
  ...
  auto M             = gf<cartesian_product<imfreq, imfreq>, scalar_valued>{{iw_mesh_large, iw_mesh_large}};

  auto iw_mesh = gf_mesh<imfreq>{1.0, Fermion, n_iw};
  auto iW_mesh = gf_mesh<imfreq>{1.0, Boson, n_iw};
  auto M4      = gf<cartesian_product<imfreq, imfreq, imfreq>, scalar_valued>{{iW_mesh, iw_mesh, iw_mesh}};
