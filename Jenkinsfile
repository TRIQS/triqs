...
for (int i = 0; i < dockerPlatforms.size(); i++) {
  ...
  platforms[platform] = { -> node('docker') {
    stage(platform) { timeout(time: 1, unit: 'HOURS') {
      ...
      """
      /* build and tag */
      def img = docker.build("flatironinstitute/${projectName}:${env.BRANCH_NAME}-${env.STAGE_NAME}", "--build-arg BUILD_DOC=${platform==documentationPlatform} --build-arg BUILD_ID=${env.BUILD_TAG} .")
    ...
    stage("osx-$platform") { timeout(time: 1, unit: 'HOURS') {
      ...
      dir(buildDir) { withEnv(platformEnv[1].collect { it.replace('\$BREW', env.BREW) } + [
        ...
        sh """#!/bin/bash -ex
          ...
          DYLD_LIBRARY_PATH=\$BREW/lib pip3 install --no-binary=mpi4py -U -r $workDir/requirements.txt
        """

        sh "cmake $workDir -DCMAKE_INSTALL_PREFIX=$installDir -DBuild_Deps=Always"
        sh "make -j3"
