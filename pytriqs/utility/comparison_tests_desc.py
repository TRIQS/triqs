...
for types in ['imfreq', 'refreq', 'imtime', 'retime', 'legendre']:
  module.add_function(name = "assert_gfs_are_close", signature = "void(gf_view<%s> x, gf_view<%s> y)"%(2*(types,)), doc = "compare two gfs")
  module.add_function(name = "assert_block_gfs_are_close", signature = "void(block_gf_view<%s> x, block_gf_view<%s> y)"%(2*(types,)), doc = "compare two block gfs")
