...
namespace triqs {
  namespace gfs {
    ...
    gf<dlr_coeffs> dlr_coeffs_from_dlr_imtime(gf_const_view<dlr_imtime> g_tau) {
      gf<dlr_coeffs> g_dlr{g_tau.mesh(), g_tau.target_shape()};
      g_dlr.data() = g_dlr.mesh().dlr_it().vals2coefs(g_tau.data());
      return g_dlr;
    }

    gf<dlr_coeffs, scalar_valued> dlr_coeffs_from_dlr_imtime(
      gf_const_view<dlr_imtime, scalar_valued> g_tau) {
      gf<dlr_coeffs, scalar_valued> g_dlr{g_tau.mesh()};
      g_dlr.data() = g_dlr.mesh().dlr_it().vals2coefs(g_tau.data());
      return g_dlr;
    }

    gf<dlr_imtime> dlr_imtime_from_dlr_coeffs(gf_const_view<dlr_coeffs> g_dlr) {
      auto g_tau = gf<dlr_imtime>{g_dlr.mesh()};
    ...
    gf<dlr_imtime, scalar_valued> dlr_imtime_from_dlr_coeffs(gf_const_view<dlr_coeffs, scalar_valued> g_dlr) {
      auto g_tau = gf<dlr_imtime, scalar_valued>{g_dlr.mesh()};
      g_tau.data() = g_dlr.mesh().dlr_it().coefs2vals(g_dlr.data());
      return g_tau;
    }
    ...
    gf<dlr_coeffs> dlr_coeffs_from_dlr_imfreq(gf_const_view<dlr_imfreq> g_iw) {
      gf<dlr_coeffs> g_dlr{g_iw.mesh(), g_iw.target_shape()};
      auto beta_inv = 1. / g_dlr.mesh().domain().beta;
      g_dlr.data() = -beta_inv * g_dlr.mesh().dlr_if().vals2coefs(g_iw.data());
      return g_dlr;
    }

    gf<dlr_coeffs, scalar_valued> dlr_coeffs_from_dlr_imfreq(
      gf_const_view<dlr_imfreq, scalar_valued> g_iw) {
      gf<dlr_coeffs, scalar_valued> g_dlr{g_iw.mesh()};
      auto beta_inv = 1. / g_dlr.mesh().domain().beta;
      g_dlr.data() = -beta_inv * g_dlr.mesh().dlr_if().vals2coefs(g_iw.data());
      return g_dlr;
    }
    ...
    gf<dlr_imfreq> dlr_imfreq_from_dlr_coeffs(gf_const_view<dlr_coeffs> g_dlr) {
      auto g_iw = gf<dlr_imfreq>{g_dlr.mesh()};
    ...
    gf<dlr_imfreq, scalar_valued> dlr_imfreq_from_dlr_coeffs(gf_const_view<dlr_coeffs, scalar_valued> g_dlr) {
      auto g_iw = gf<dlr_imfreq, scalar_valued>{g_dlr.mesh()};
      auto beta = g_dlr.mesh().domain().beta;
      g_iw.data() = -beta * g_dlr.mesh().dlr_if().coefs2vals(g_dlr.data());
      return g_iw;
    }
