...
#define IMPL(z, RR, unused)                                                                                                                          \
  template <typename FntType> FORCEINLINE void foreach_impl(_traversal_c, domain_t<RR> const &dom, memory_layout_t<RR> const &ml, FntType F) {       \
    const mini_vector<foreach_int_type, RR> l(dom.lengths());                                                                                        \
    BOOST_PP_REPEAT(RR, AUX_C, nil) { F(BOOST_PP_REPEAT(RR, AUX3C, nil)); }                                                                          \
  }                                                                                                                                                  \
  template <typename FntType> FORCEINLINE void foreach_impl(_traversal_fortran, domain_t<RR> const &dom, memory_layout_t<RR> ml, FntType F) {        \
    foreach_int_type ind[RR];                                                                                                                        \
    const mini_vector<foreach_int_type, RR> l(dom.lengths());                                                                                        \
    BOOST_PP_REPEAT(RR, AUX_F, BOOST_PP_DEC(RR)) { F(BOOST_PP_REPEAT(RR, AUX3, nil)); }                                                              \
  }                                                                                                                                                  \
  template <typename FntType> FORCEINLINE void foreach_impl(_traversal_dynamical, domain_t<RR> const &dom, memory_layout_t<RR> ml, FntType F) {      \
    foreach_int_type ind[RR];                                                                                                                        \
    const mini_vector<foreach_int_type, RR> l(dom.lengths());                                                                                        \
...
#undef AUX3C
    ...
    template <typename A, typename Enable = void> struct _get_traversal_order {
      ...
      static memory_layout_t<A::domain_type::rank> invoke(A const &a) { return memory_layout_t<A::domain_type::rank>{}; }
    };
    ...
    template <typename T, typename Function> FORCEINLINE void _foreach_on_indexmap(T const &x, Function const &F) {
