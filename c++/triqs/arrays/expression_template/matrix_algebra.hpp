...
namespace triqs {
  namespace arrays {
    ...
    template <typename A, typename B> typename _matmul_rvalue<A, B>::type operator*(A const &a, B const &b) {
    ...
    template <typename M, typename V> typename _mat_vec_mul_rvalue<M, V>::type operator*(M const &m, V const &v) {
