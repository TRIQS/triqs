...
/**
 * @file
 * @brief Macros that define a legacy (pre C++20) concept-tag trait pair.
 */
...
/// Helper macro that produces the name of the tag for @ref TRIQS_DEFINE_CONCEPT_AND_ASSOCIATED_TRAIT.
#define TRIQS_CONCEPT_TAG_NAME(MyBeautifulConcept) MyBeautifulConcept##__concept_tag

/**
 * @brief Define a tag-based pseudo-concept.
 *
 * @details It does the following:
 * 
 * - Defines an empty tag struct using @ref TRIQS_CONCEPT_TAG_NAME.
 * - Defines a class template `MyBeautifulConcept<T>` derived from `std::is_base_of<tag, T>` and partial specializations 
 * that strip cv- and reference-qualifiers.
...
/// Helper used by @ref TRIQS_DEFINE_CONCEPT_AND_ASSOCIATED_TRAIT_R.
#define TRIQS_DEFINE_CONCEPT_AND_ASSOCIATED_TRAIT_R_AUX(r, data, i, elem) BOOST_PP_COMMA_IF(i) TRIQS_CONCEPT_TAG_NAME(elem)

/**
 * @brief Define a tag-based pseudo-concept with refinements.
