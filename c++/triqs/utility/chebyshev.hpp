...
namespace triqs::utility {
  ...
  /**
   * @brief Compute Chebyshev points of the first kind on the interval [-1, 1].
   *
   ...
   * @return Vector of N Chebyshev points in ascending order on [-1, 1].
   */
  inline nda::vector<double> chebyshev_points(long N) {
    ...
    for (long i = 0; i < N; ++i) {
      ...
      points[N - 1 - i] = std::cos((2.0 * i + 1.0) * pi / (2.0 * N));
    }
    return points;
  }
   ...
   *
   * @param N Number of Chebyshev points.
   * @return Vector of N barycentric weights.
   */
  inline nda::vector<double> chebyshev_barycentric_weights(long N) {
    ...
    for (long i = 0; i < N; ++i) {
      ...
      weights[N - 1 - i] = sign * std::sin((2.0 * i + 1.0) * pi / (2.0 * N));
    }
    return weights;
  }

  /**
   * @brief Scale a value from interval [a, b] to the standard interval [-1, 1].
   *
   * @param x Value in [a, b].
