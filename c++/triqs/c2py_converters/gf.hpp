...
#include "fmt/base.h"
  ...
  #error "This converter is meant to be automatically included when c2py is included
#endif
...
namespace c2py {
  ...
  namespace detail {
    ...
    inline void set_err_impl(PyObject *p, std::string_view kind, const char *X, std::string const &self_type, std::string const &C) {
      ...
      err << "Cpp2py converter: Python to C++ :\n"
          ...
          << "  ... Cannot convert the " << X << " of " << kind
          << " from Python type :  " << p->ob_type->tp_name << " to the C++ type " << C;
  ...
  // -----------------------------------
  //    is_view
  //  One MUST declare hat a type is a view.
  // -----------------------------------
