...
namespace triqs::mesh {
  ...
  class C2PY_IGNORE tail_fitter {
     ...
     */
    template <typename M> int n_pts_in_tail(M const &m) const { return std::min(static_cast<int>(std::round(r_ * m.size() / 2)), p_max_); }

    /// Get the fraction \f$ r \f$ of the mesh to be considered for the tail fit.
    double get_tail_fraction() const { return r_; }
    ...
    template <typename M> auto get_tail_fit_indices(M const &m) {
      ...
      auto const p_r = static_cast<int>(std::round(r_ * m.size() / 2));
    ...
    template <bool enforce_hermiticity = false, typename M> void setup_lss(M const &m, int n_A) {
      ...
      if (fit_idxs_.empty()) fit_idxs_ = get_tail_fit_indices(m);
      ...
      if (!adjust_q_) {
        ...
        long q_max = std::min(static_cast<long>(q_max_), static_cast<long>(1. + 16. / std::log10(1 + std::abs(m.w_max()))));
        // we try to use at least two times as many data points as we have unknown coefficients
        q_max = std::min(q_max, V_.extent(0) / 2);
        for (long q = q_max; q >= n_A; --q) {
          auto ptr = worker_factory(q);
          if (ptr->S_vec()[ptr->S_vec().size() - 1] > rcond_) {
    ...
    auto fit(M const &m, nda::array_const_view<std::complex<double>, R> D, bool rescale, nda::array_const_view<std::complex<double>, R> C,
      ...
      if (n_A > q_) return std::pair<nda::array<std::complex<double>, R>, double>{C, 0.0};
      ...
      auto D_mat       = nda::matrix<std::complex<double>>(V_.extent(0), ncols);
      for (auto [i, n] : itertools::enumerate(fit_idxs_)) {
        if constexpr (R == 1) {
      ...
      if (n_A > 0) {
        ...
        D_mat -= V_(nda::range::all, nda::range(n_A)) * C_mat;
      }
    ...
    private:
    static constexpr int q_max_    = 9;
    static constexpr double rcond_ = 1e-8;
    double r_;
    int p_max_;
    bool adjust_q_;
    int q_;
    std::array<std::unique_ptr<const nda::lapack::gelss_worker<dcomplex>>, q_max_ + 1> lss_;
    std::array<std::unique_ptr<const nda::lapack::gelss_worker_hermitian>, q_max_ + 1> lss_hermitian_;
    nda::matrix<dcomplex> V_;
    std::vector<long> fit_idxs_;
  };
  ...
  class C2PY_IGNORE tail_fitter_handle {
    ...
    private:
    mutable std::shared_ptr<tail_fitter> tf_ptr_;
  };
