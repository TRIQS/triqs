...
//
// Authors: Olivier Parcollet, Nils Wentzell, Hugo Strand
...
namespace triqs::mesh {
  ...
  struct dlr_imfreq {
     ...
     */
    dlr_imfreq(double beta, statistic_enum statistic, double w_max, double eps)
       : dlr_imfreq(beta, statistic, w_max, eps, cppdlr::build_dlr_rf(w_max * beta, eps)) {}
    ...
    friend std::ostream &operator<<(std::ostream &sout, dlr_imfreq const &m) {
      ...
      return sout << fmt::format("DLR imfreq mesh of size {} with beta = {}, statistic = {}, w_max = {}, eps = {}", m._beta, m.size(), stat_cstr,
                                 m._w_max, m._eps);
    }
    ...
    friend void h5_read(h5::group fg, std::string const &subgroup_name, dlr_imfreq &m) {
      ...
      m              = dlr_imfreq(beta, statistic, w_max, eps, {_dlr_freq, _dlr_it, _dlr_if});
    }
  };
