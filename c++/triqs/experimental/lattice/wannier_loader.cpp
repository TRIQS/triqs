...
namespace triqs::experimental::lattice {
  ...
  read_wannier90_tb_data(std::string const &w90_path_and_seedname) {
    ...
    for ([[maybe_unused]] auto i : detail::read_N<std::string>(file, 5)) {}
    ...
    int n_r       = detail::read<int>(file);

    // read in the R vector degeneracies
    std::vector<int> r_degeneracies;
    for (auto i : detail::read_N<int>(file, n_r)) { r_degeneracies.push_back(i); };
    ...
    for (auto ir : nda::range(n_r)) {
      ...
      for ([[maybe_unused]] auto [x, y] : product(nda::range(n_wannier), nda::range(n_wannier))) {
        ...
        // check state
        // NB if we want to use ws_cells vec data, we will likely want to use all wannier indices instead
  ...
  read_wannier90_hr_data(std::string const &w90_path_and_seedname) {
    ...
    std::ifstream file(hr_filename);
    if (!file.is_open()) throw "Could not open *_hr.dat file with provided seedname: " + w90_path_and_seedname;
    ...
    return {std::move(r_vectors), std::move(H_r)};
  }
} // namespace triqs::experimental::lattice
