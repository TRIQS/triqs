...
#define ATOM_DIAG_T typename atom_diag<Complex>
  ...
  inline void check_lehmann_struct([[maybe_unused]] gf_lehmann_t<Complex> const &lehmann, [[maybe_unused]] block_gf_view<T> g) {
#ifndef NDEBUG
  ...
  template <bool Complex>
  block_gf<imfreq> atomic_g_iw(ATOM_DIAG const &atom, double beta, gf_struct_t const &gf_struct, int n_iw, excluded_states_t const &excluded_states) {
  ...
  template <bool Complex>
  block_gf<legendre> atomic_g_l(ATOM_DIAG const &atom, double beta, gf_struct_t const &gf_struct, int n_l, excluded_states_t const &excluded_states) {
