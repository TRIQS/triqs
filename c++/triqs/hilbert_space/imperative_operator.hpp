...
namespace triqs::hilbert_space {
   ...
   */
  template <typename HS, typename T = double, bool UseMap = false> class imperative_operator {
    private:
     ...
     * describing how the operator maps subspaces to one another must be supplied through `hmap` and `subspaces`.
     * When `UseMap = false`, `hmap` must be empty and `subspaces` must be `nullptr`; an internal error is raised
     * otherwise.
     *
     ...
     * @param subspaces Pointer to the vector of all Hilbert subspaces referenced by `hmap` (only used when
     * `UseMap = true`).
     */
    imperative_operator(triqs::operators::many_body_operator_generic<coeff_t> const &op, fundamental_operator_set const &fops,
      ...
      for (auto const &term : op) {
        ...
        terms_.push_back(one_term_t{coeff_t(coef), d_mask, dag_mask, d_count_mask, dag_count_mask});
      }
    }
     ...
     * @param f Callable object applied to each coefficient of the operator.
     */
    template <typename F> void update(F f) {
      for (auto &t_i : terms_) f(t_i.coeff);
    }
     ...
     * `T` is itself a callable (e.g. for parametric or time-dependent operators). For ordinary scalar coefficients the 
     * arguments are simply ignored.
     *
     * @tparam S Many-body state type.
     * @tparam Args Types of the optional extra arguments forwarded to the coefficients.
     * @param psi Initial many-body state \f$ \lvert \psi \rangle \f$.
     * @param args Optional arguments forwarded to each coefficient of the operator (only meaningful when `T` is a 
     * callable).
     * @return Resulting many-body state \f$ \hat{O} \lvert \psi \rangle = \lvert \psi' \rangle  \f$.
     */
    template <typename S, typename... Args> S operator()(S const &psi, Args &&...args) const {

      S target_st    = get_target_st(psi);
      auto const &hs = psi.get_hilbert();

      using amplitude_t = typename S::value_type;
    ...
    template <typename S> S get_target_st(S const &psi) const {
      if constexpr (UseMap) {
        auto n = hmap_[psi.get_hilbert().get_index()];
        if (n == -1) return S{};
        return S{(*subspaces_)[n]};
      } else {
        return S(psi.get_hilbert());
      }
    }
